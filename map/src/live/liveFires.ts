 ...
 *
 * The colours are spec.md §4.7's `active`, `risk` and `contained`, unchanged.
 *
 * `contained` is the honest label for a cluster Deepfire still flags active but that no
 * satellite has seen for hours: it is light blue, not warm, because it is no longer
 * asking for anyone's attention. Dropping those instead would leave the screen blank on
 * a quiet day, which lies just as much, only by omission.
 */

export type LiveStatus = 'active' | 'contained' | 'risk';
...
export function statusFromLiveCells(
  ...
  containedCellIds: readonly string[],
  riskCellIds: readonly string[],
): ReadonlyMap<string, LiveStatus> {
  const statuses = new Map<string, LiveStatus>();
  // Set in order of increasing urgency: the last write wins, so a cell claimed by two
  // sources ends up showing the most serious of them.
  for (const id of riskCellIds) statuses.set(id, 'risk');
  for (const id of containedCellIds) statuses.set(id, 'contained');
